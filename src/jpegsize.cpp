...
2006-07-18 [OS3, OS3 srl]
	Adapted to work with FlyPDF
...
#define ERREXIT(msg)	throw PdfException ( msg )
...
scan_JPEG_header (FILE *infile, CPDFimageInfo *jInfo)
{
	...
	do_log ( "scan_JPEG_header 1" );

  /* Expect SOI at start of file */
  if (first_marker(infile) != M_SOI)
	return(-2);

  do_log ( "scan_JPEG_header 2" );
...
FILE *infile;		/* input JPEG file */
	...
	do_log ( "read_JPEG_header 1" );

    /* Open the input file. */
    if ((infile = fopen(filename, READ_BINARY)) == NULL) {
		do_log ( "read_JPEG_header 2" );
      	fprintf(stderr, "FlyPDF read_JPEG_header\nCan't open image file: %s", filename);
		do_log ( "read_JPEG_header 3" );
      	return(-1);
    }

	do_log ( "read_JPEG_header 4" );

    /* Scan the JPEG headers. */
    errcode =  scan_JPEG_header(infile, jInfo);
	do_log ( "read_JPEG_header 5" );
    fclose(infile);
	do_log ( "read_JPEG_header 6" );
    return(errcode);
}
